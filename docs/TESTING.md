...
- [ ] Every compose row that writes notes has an instrument picker beside it (MIDI import, Type a melody, Type the words). Point Type the words at **Bass** and press Sing it: the bass grid fills, the bass comes on, the melody is untouched, and the line under the buttons starts "Bass sings…". The kit is not in the melody/words pickers — it has no note names to type or sing.
