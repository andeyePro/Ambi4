...
- [ ] **[BLOCKED — fromClaude, only he can create the repo]** **Move the paid features into a private repo** (the history purge half is DONE — third-round section below). His item 86 asks whether the where-should-it-live conversation (inside andeyePro's existing repo versus a new repo, and the naming convention if new) actually happened: it has not — that was my label on unexamined thinking. OWED: a written assessment of andeyePro-repo versus separate-private-repo with a naming recommendation, delivered in fromClaude, before he creates anything. Testing hidden features moves to multiple accounts rather than a console flag.
...
- [ ] **The Astro 5→7 migration** — the root fix for all 10 Dependabot advisories (investigated 2026-07-29, `docs/dependabot-2026-07-29.md`; nine inert today, the define:vars one becomes live if unmoderated preset slugs ever compile in). Deliberate migration: upgrade to astro ≥7.1.4, full engine suite, every browser drive, layout sweep, `tests/audio-reference.mjs` unchanged. Never `npm audit fix --force`. Timing awaits the owner's answer to fromClaude 93 (now versus nearer release)
