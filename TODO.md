...
- [ ] **A click on note onset — `fingered` bass, and bossa generally.** Reported twice, independently ("Soul Groove - Fingered bass is also cracking on note start", "Bossa … has a lot of sounds that are starting with a click, due to inadvertant sound design issues").
  ...
  **So the click is an interaction, not a voice.** The remaining candidates, in order: a release tail colliding with the next onset on a fast line (`fingered`'s release is 0.14 s); the `takeOver` slur path, which the harness could not exercise — a second note 120 ms after the first rendered **silence** for `fingered`, which is either a real bug in note takeover or a gap in how the harness constructs its note objects, and is the first thing to settle; and the summing of several tracks at once, which no single-voice render can show. Extend `onset-render.mjs` down that path rather than starting again.
