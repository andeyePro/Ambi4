...
- [ ] **The sawing is the `call` voice — confirmed by the owner (item 85).** "I heard the sawing again in Synthwave, and saw that Texture was set to Call; I have now picked Call again and hear the sawing." My earlier note ("no genre selects call by default, so it cannot be the one") missed that he had SET Texture to Call himself. The v0.0.80 wash fix was a different, also-real layer. Work: render `call` offline through the wash-sweep-render harness pattern, measure what saws (its own filter movement on noise is the prime suspect, same physics as wash), fix it, and keep whatever character makes call call — same discipline as v0.0.80: spectral-centroid travel measured before and after, not listened for.
